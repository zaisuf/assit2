...
export default function RenderUiDesign({ designId, onFetchedContent }: RenderUiDesignProps) {
  ...
  const startWebRTCVADRecording = () => {
    // Show a user-friendly prompt before requesting mic access
      ...
      return (
        ...
        >
          <div className="flex flex-row items-start" style={{position:'relative'}}>
            {/* Fetched Content Box as floating top-left */}
            {fetchedContent && (
              <div
                className="p-4 bg-emerald-900/90 text-white rounded-xl border border-emerald-400 animate-fade-in shadow-lg"
                style={{
                  position: 'absolute',
                  top: -90,
                  left: -320,
                ...
                {messages.map((message, index) => (
                  <div key={index} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                    <div
                      className={`max-w-[80%] ${
                        message.role === 'user'
                          ? (config.userQueryCardColor?.backgroundColor === 'custom'
                            ? ''
                            : config.userQueryCardColor?.backgroundColor || 'bg-white/10')
                          : (config.selectedResponseCardStyle?.className || '')
                      } ${config.selectedResponseCardStyle?.shape || ''}`}
                      style={{
                        ...(config.selectedTextColor === 'custom' && config.customTextColor ? { color: config.customTextColor } : {}),
                        ...(message.role === 'user' && config.userQueryCardColor?.backgroundColor === 'custom' ? { backgroundColor: config.userQueryCardColor.customColor } : {})
                      }}
                    >
