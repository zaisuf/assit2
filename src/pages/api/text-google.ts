...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    // Supported models and their API versions
    const modelConfig = {
      "gemini-1.5-flash-latest": { version: "v1beta" },
      "gemini-1.5-pro": { version: "v1beta" }
    };
    
    const supportedModels = Object.keys(modelConfig);
    ...
    const version = modelConfig[model as keyof typeof modelConfig]?.version || "v1beta";
    const apiRes = await fetch(`https://generativelanguage.googleapis.com/${version}/models/${model}:generateContent?key=${GOOGLE_GEMINI_API_KEY}`, {
      method: "POST",
