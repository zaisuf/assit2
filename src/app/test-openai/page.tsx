...
const TestOpenAIChat: React.FC = () => {
  ...
  const handleSendMessage = async (text: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        // Special handling for OpenAI quota exceeded error
        if (res.status === 429 && data?.error?.includes("quota exceeded")) {
          setMessages((prev) => [
            ...prev,
            {
              role: "assistant",
              content: `OpenAI API quota exceeded. Please check your plan and billing details.\n\n[More info](https://platform.openai.com/docs/guides/error-codes/api-errors)`,
              timestamp: new Date(),
            },
          ]);
          return;
      ...
      <div className="p-4 bg-black/20 border-b border-white/10">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1">
            <h3 className="text-white text-xl font-semibold">OpenAI Chatbot</h3>
            <p className="text-white/60 text-sm">Test OpenAI API with chat UI</p>
          </div>
          <div>
            <select
