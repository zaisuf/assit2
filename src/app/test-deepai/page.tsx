...
const TestDeepAIChat: React.FC = () => {
  ...
  const handleSendMessage = async (text: string) => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch("/api/test-deepai", {
        method: "POST",
      ...
      <div className="p-4 bg-black/20 border-b border-white/10">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1">
            <h3 className="text-white text-xl font-semibold">DeepAI Chatbot</h3>
            <p className="text-white/60 text-sm">Test DeepAI Text Generator API with chat UI</p>
          </div>
          <div>
            <select
