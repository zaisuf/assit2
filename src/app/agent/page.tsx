...
export default function AgentChatPage() {
  // Fix for TypeScript and strict mode: declare window.chatbase, avoid 'arguments', type rest params, ignore script.domain
  React.useEffect(() => {
    // @ts-ignore
    if (!window.chatbase || window.chatbase("getState") !== "initialized") {
      // @ts-ignore
      window.chatbase = (...params: any[]) => {
        // @ts-ignore
    ...
    const onLoad = function () {
      if (!document.getElementById("LD_YJiWGLyS1-4SXcM8lw")) {
        const script = document.createElement("script");
        script.src = "https://www.chatbase.co/embed.min.js";
        script.id = "LD_YJiWGLyS1-4SXcM8lw";
        // @ts-ignore
  ...
  const handleGoToSdk = (designId: string) => {
    router.push(`/agent/sdk?uiId=${designId}`);
  };
                ...
                <table className="min-w-full text-white/90 rounded-lg text-base font-syne">
                  <thead>
                    <tr>
                      ...
                      <th className="px-4 py-2 text-left" style={{color:'#A0A0A0', fontWeight:400}}>SDK</th>
                    ...
                    {partners.map((p) => (
                      <tr key={p.id}>
                        ...
                        <td className="px-4 py-2">
                          <button
                            onClick={() => handleGoToSdk(p.designId)}
