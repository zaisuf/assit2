...
const ModernAIAssistantElevenLabs: React.FC = () => {
  ...
  // State for editable Assistant Bot text and user query card color
  const [assistantBotText, setAssistantBotText] = useState('Assistant Bot');
  const [userQueryCardColor, setUserQueryCardColor] = useState({ backgroundColor: 'bg-white/10', customColor: '' });
  ...
  const [customLoadingColor, setCustomLoadingColor] = useState("#10b981"); // Default teal color
  const [selectedResponseCardStyle, setSelectedResponseCardStyle] = useState<ResponseCardStyle>({
    ...RESPONSE_CARD_STYLES[0],
    backgroundColor: RESPONSE_CARD_STYLES[0].backgroundColor || 'bg-white/10',
    customBackgroundColor: ''
  });
    ...
    const fetchUiConfig = async () => {
      ...
      try {
        ...
        if (snap.exists()) {
          ...
          if (data.selectedFooterColor || data.customFooterColor) {
            // If there's a custom color and selectedFooterColor is 'custom', use the custom color
            if (data.customFooterColor && (!data.selectedFooterColor || data.selectedFooterColor === 'custom')) {
              setSelectedFooterColor('custom');
              setCustomFooterColor(data.customFooterColor);
            } else {
              // Otherwise use the selected color from the predefined options
              setSelectedFooterColor(data.selectedFooterColor);
              setCustomFooterColor(''); // Clear custom color when using predefined
            }
          }
          ...
          // Load send button and icon styles
          if (data.sendButtonStyle) {
            const style = SEND_BUTTON_STYLES.find(s => s.name === data.sendButtonStyle);
          ...
          if (data.sendIconStyle) {
            const icon = SEND_ICONS.find(i => i.name === data.sendIconStyle);
            if (icon) setSelectedSendIcon(icon);
          }
          
          // Load box dimensions
          if (data.boxWidth) setChatbotBoxWidth(data.boxWidth);
          if (data.boxHeight) setChatbotBoxHeight(data.boxHeight);
          
          // Load UI shape
          if (data.selectedUiShape) setSelectedUiShape(data.selectedUiShape);

          // Load New Chat Icon Style
          if (data.newChatIconStyle) {
            const newChatIcon = NEW_CHAT_ICONS.find(i => i.name === data.newChatIconStyle);
            if (newChatIcon) setSelectedNewChatIcon(newChatIcon);
          }

          // Load Down Arrow Icon Style
          if (data.downArrowIconStyle) {
            const downArrowIcon = DOWN_ARROW_ICONS.find(i => i.name === data.downArrowIconStyle);
            if (downArrowIcon) setSelectedDownArrowIcon(downArrowIcon);
          }
          ...
          if (data.selectedResponseCardStyle) {
            ...
            if (style) {
              // Create a new style object with saved background colors
              const savedStyle = {
                ...style,
                backgroundColor: data.responseCardBackgroundColor || style.backgroundColor,
                customBackgroundColor: data.responseCardCustomBackgroundColor || style.customBackgroundColor
              };
              setSelectedResponseCardStyle(savedStyle);
            }
          }
          
          // Load custom loading color
          if (data.loadingColor) {
            setCustomLoadingColor(data.loadingColor);
          }

          // Load user query card colors
          if (data.userQueryCardBackgroundColor || data.userQueryCardCustomColor) {
            setUserQueryCardColor({
              backgroundColor: data.userQueryCardBackgroundColor || 'bg-white/10',
              customColor: data.userQueryCardCustomColor || ''
            });
          }
          ...
          // Load custom new chat icon color
          if (data.customNewChatIconColor) {
            setCustomNewChatIconColor(data.customNewChatIconColor);
          }
      ...
      setSelectedResponseCardStyle((prev) => {
        if (!prev || prev.name === prev.templateName) {
          return {
            ...
            backgroundColor: prev && typeof prev.backgroundColor !== "undefined" ? prev.backgroundColor : "bg-white/10",
            customBackgroundColor: prev && typeof prev.customBackgroundColor !== "undefined" ? prev.customBackgroundColor : "",
          };
        }
        return prev;
      });
      ...
      setSelectedResponseCardStyle((prev) => {
        if (!prev || prev.name === prev.templateName) {
          return {
            ...
            shape: selectedUiTemplate.card.match(/rounded-[^\s]+|\[border-radius:[^\]]+\]/)?.[0] || "rounded-xl",
            templateName: selectedUiTemplate.name,
            backgroundColor: selectedUiTemplate.bg || "bg-white/10",
            customBackgroundColor: "",
          };
        }
        return prev;
...
const [selectedDownArrowIcon, setSelectedDownArrowIcon] = useState(DOWN_ARROW_ICONS[0]);
  ...
  const handleSaveUiConfig = async () => {
    ...
    // If selectedBgColor is set, clear customBgColor before saving
    let configCustomBgColor = customBgColor;
    if (selectedBgColor) {
      configCustomBgColor = '';
    }
    
    // If selectedFooterColor is set, clear customFooterColor before saving
    let configCustomFooterColor = customFooterColor;
    if (selectedFooterColor && selectedFooterColor !== 'custom') {
      configCustomFooterColor = '';
    }

    const designConfig = {
      selectedUiTemplate: selectedUiTemplate?.name,
      selectedBgColor,
      customBgColor: configCustomBgColor,
      selectedHeaderStyle,
      selectedHeaderColor,
      customHeaderColor,
      selectedFooterStyle,
      selectedFooterColor,
      customFooterColor: configCustomFooterColor,
      userQueryCardColor,
      selectedInputBarStyle: selectedInputBarStyle?.name,
      selectedInputBarColor,
      customInputBarColor,
      inputPlaceholder,
      // Chat icon and button styles
      sendIconStyle: selectedSendIcon?.name || 'Send (Default)',
      sendButtonStyle: selectedSendButtonStyle?.name || 'Emerald Solid',
      // Box dimensions
      boxWidth: chatbotBoxWidth || 450,
      boxHeight: chatbotBoxHeight || 500,
      // UI shape (using only selectedUiShape for consistency)
      selectedUiShape: selectedUiShape || 'rounded-xl',
      // New Chat and Down Arrow Icons
      newChatIconStyle: selectedNewChatIcon?.name || 'New',
      downArrowIconStyle: selectedDownArrowIcon?.name || 'Default',
      // Add new values to save
      chatbotBoxWidth,
      chatbotBoxHeight,
      selectedSendButtonStyle: selectedSendButtonStyle?.name,
      customSendButtonColor,
      selectedSendIcon: selectedSendIcon?.name,
      customNewChatIconColor, // Add custom icon color
      selectedTextStyle,
      selectedTextColor,
      customTextColor,
      selectedLanguage,
      ...
      customLoadingColor,
      selectedResponseCardStyle: {
        name: selectedResponseCardStyle?.name || 'Classic',
        backgroundColor: selectedResponseCardStyle?.backgroundColor || 'bg-white/10',
        customBackgroundColor: selectedResponseCardStyle?.customBackgroundColor || '',
        className: selectedResponseCardStyle?.className || 'bg-white/10 border border-white/20 rounded-xl shadow-lg',
        bubbleClass: selectedResponseCardStyle?.bubbleClass || '',
        shape: selectedResponseCardStyle?.shape || 'rounded-xl',
        textClass: selectedResponseCardStyle?.textClass || 'text-white',
        ...(selectedResponseCardStyle?.templateName ? { templateName: selectedResponseCardStyle.templateName } : {})
      },
      responseCardBackgroundColor: selectedResponseCardStyle?.backgroundColor || 'bg-white/10',
      responseCardCustomBackgroundColor: selectedResponseCardStyle?.customBackgroundColor || '',
      userQueryCardBackgroundColor: userQueryCardColor?.backgroundColor,
      userQueryCardCustomColor: userQueryCardColor?.customColor,
    ...
    try {
      ...
      console.log('Saving design with ID:', uiId);
        ...
        <div className="flex flex-col items-center mt-4 w-full gap-2">
          ...
          <div className="grid grid-cols-2 gap-2 w-full mb-2">
            {[
              ...
              { label: 'Pill', value: '!rounded-[2rem]', preview: 'rounded-full border border-white/40 w-16 h-6' },
              ...
              <div className="flex flex-col items-center mt-4 w-full gap-2">
                ...
                <div className="w-full mb-4">
                  <div className="flex items-center justify-between">
                    <label className="text-white/80 text-xs">Upload Background Image</label>
                    <label className="px-2 py-1 text-xs bg-white/10 rounded border border-white/20 cursor-pointer hover:bg-white/20">
                      Upload
                      <input
                        type="file"
                        accept="image/*"
                  ...
                  {selectedBgColor && selectedBgColor.startsWith('data:image') && (
                    <div className="flex items-center gap-2 mt-2">
                      <div className="w-4 h-4 rounded border border-white/20 overflow-hidden bg-white/10 flex items-center justify-center">
                        <Image src={selectedBgColor} alt="Background Preview" className="object-cover w-full h-full" width={16} height={16} />
                      </div>
                      <button
                        className="px-1 py-0.5 text-[10px] rounded bg-red-500/70 text-white hover:bg-red-600 flex-shrink-0"
              ...
              <div className="flex flex-col items-center mt-4 w-full gap-4">
                ...
                <div className="w-full mb-2">
                  <div className="flex items-center justify-between">
                    <label className="text-white/80 text-xs">Upload Header Logo</label>
                    <label className="px-2 py-1 text-xs bg-white/10 rounded border border-white/20 cursor-pointer hover:bg-white/20">
                      Upload
                      <input
                        type="file"
                  ...
                  {customHeaderLogo && (
                    <div className="flex items-center gap-2 mt-2">
                      <div className="w-4 h-4 rounded-full border border-white/20 overflow-hidden bg-white/10 flex items-center justify-center">
                        <Image src={customHeaderLogo} alt="Header Logo Preview" className="object-cover w-full h-full rounded-full" width={16} height={16} />
                      </div>
                      <button
                        className="px-1 py-0.5 text-[10px] rounded bg-red-500/70 text-white hover:bg-red-600 flex-shrink-0"
              ...
              <div className="flex flex-col items-center mt-4 w-full gap-4">
                <div className="mb-4">
                  <span className="text-white/80 text-xs mb-2 block">Loading Animation Color</span>
                  <div className="flex items-center gap-2">
                    <input
                      type="color"
                      value={customLoadingColor}
                      onChange={e => setCustomLoadingColor(e.target.value)}
                      className="w-8 h-8 rounded border border-white/20 bg-transparent cursor-pointer"
                    />
                    <span className="text-white/80 text-xs">Custom Color</span>
                  </div>
                ...
                <div className="flex flex-col gap-2 w-full">
                  {LOADING_ANIMATIONS.map((anim) => (
                    <button
                      ...
                      type="button"
                    >
                      <span className="inline-block">{anim.component ? anim.component({ color: '#10b981' }) : null}</span>
                      {anim.name}
                    </button>
                  ))}
                </div>
              ...
              <div className="flex flex-col items-center mt-4 w-full gap-4">
                {/* Response Card Background Color */}
                <div className="w-full mb-4">
                  <span className="text-white/80 text-xs mb-2 block">Response Card Background</span>
                  <div className="flex flex-col gap-2">
                    <div className="flex items-center gap-2">
                      <input
                        type="color"
                        value={selectedResponseCardStyle.customBackgroundColor || "#ffffff"}
                        onChange={e => setSelectedResponseCardStyle(prev => ({
                          ...prev,
                ...
                <div className="w-full mb-4">
                  <span className="text-white/80 text-xs mb-2 block">User Query Card Colors</span>
                  <div className="flex flex-wrap gap-2">
                    <div className="flex items-center gap-2 p-2 rounded border border-white/20">
                      <input
                        type="color"
                        value={userQueryCardColor.customColor || '#ffffff'}
                        onChange={e => setUserQueryCardColor({
                          backgroundColor: 'custom',
                ...
                <div className="flex flex-col gap-2 w-full">
                  {RESPONSE_CARD_STYLES.map((style) => (
                    <button
                      ...
                      onClick={() => setSelectedResponseCardStyle(prev => ({
                        ...style,
      ...
      <div className="flex-1 flex justify-center items-center">
        <div
          className={`relative z-10 flex flex-col overflow-hidden shadow-2xl border border-gray-200
            ${selectedUiShape?.replace('!', '') || 'rounded-xl'}
        ...
        >
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4" style={{ fontSize: getResponsiveSize(16) }}>
            {messages.map((msg, idx) => (
              ...
              >
                {/* Logo moved inside the message bubble */}
                <div
                  className={`relative max-w-[80%] ${msg.role === "user" ? "ml-auto" : ""} 
                    ${selectedResponseCardStyle.className || ''} 
                    ${selectedResponseCardStyle.bubbleClass || ''} 
                    ${selectedResponseCardStyle.shape || 'rounded-xl'}`}
                  style={{
                    background: msg.role === "user" 
                      ? (userQueryCardColor.backgroundColor === 'custom'
                          ? userQueryCardColor.customColor
                          : userQueryCardColor.backgroundColor === 'bg-white/10'
                            ? 'rgba(255, 255, 255, 0.1)'
                            : userQueryCardColor.backgroundColor === 'bg-gray-900'
                              ? '#111827'
                              : '#10b981')
                      : (selectedResponseCardStyle.backgroundColor === 'custom' 
                          ? selectedResponseCardStyle.customBackgroundColor 
                          : selectedResponseCardStyle.backgroundColor || "#fff"
                    ),
                    color: msg.role === "user" 
                      ? "#fff" 
                      : (selectedResponseCardStyle.backgroundColor === '#1a1a1a' ? "#fff" : "#222"),
                    padding: "10px 16px",
                ...
                >
                  <div className="flex items-start gap-3">
                    {msg.role === "assistant" && showResponseLogo && (
                      <div className="flex-shrink-0 w-4 h-4 rounded-full bg-black/30 backdrop-blur flex items-center justify-center">
                        {customHeaderLogo ? (
                          <Image src={customHeaderLogo} alt="Response Logo" style={{objectFit: 'cover', borderRadius: '50%'}} width={18} height={18} />
                        ) : (
                          <Image src="/favicon.png" alt="Response Logo" style={{objectFit: 'contain'}} width={15} height={15} />
                        )}
                      </div>
                    )}
                    <div
                      className={`flex-1 ${getTextStyleClass(selectedTextStyle)}`}
                      style={{
                        fontWeight: selectedTextStyle === "Bold" ? 700 : 400,
                        fontStyle: selectedTextStyle === "Italic" ? "italic" : undefined,
                        textDecoration: selectedTextStyle === "Underline" ? "underline" : selectedTextStyle === "Strikethrough" ? "line-through" : undefined,
                        fontFamily:
                          selectedTextStyle === "Monospace" ? "monospace" :
                  ...
                  >
                    {msg.role === "assistant" ? (
                      <ReactMarkdown
                        components={{
                          h1: ({node, ...props}) => <h1 style={{fontWeight:700, fontSize:14, marginBottom:4}} {...props} />,
                          h2: ({node, ...props}) => <h2 style={{fontWeight:700, fontSize:14, marginBottom:3}} {...props} />,
                          h3: ({node, ...props}) => <h3 style={{fontWeight:600, fontSize:14, marginBottom:2}} {...props} />,
                          p: ({node, ...props}) => <p style={{fontWeight:400, fontSize:14, marginBottom:2}} {...props} />,
                          ul: ({node, ...props}) => <ul style={{marginLeft:16, marginBottom:2}} {...props} />,
                          ol: ({node, ...props}) => <ol style={{marginLeft:16, marginBottom:2}} {...props} />,
                          li: ({node, ...props}) => <li style={{marginBottom:1}} {...props} />,
                          table: ({node, ...props}) => <table style={{borderCollapse:'collapse', width:'100%', marginBottom:4}} {...props} />,
                          th: ({node, ...props}) => <th style={{border:'1px solid #ccc', padding:'4px', background:'#222', color:'#fff'}} {...props} />,
                          td: ({node, ...props}) => <td style={{border:'1px solid #ccc', padding:'4px'}} {...props} />,
              ...
              <div className="flex items-start justify-start">
                <div 
                  className={`max-w-[80%] 
                    ...
                    p-4 shadow-sm`}
                  style={{
                    background: selectedResponseCardStyle.backgroundColor === 'custom' 
                      ? selectedResponseCardStyle.customBackgroundColor 
                      : selectedResponseCardStyle.backgroundColor || "#fff",
                    color: selectedResponseCardStyle.backgroundColor === '#1a1a1a' ? "#fff" : "#222"
                  }}>
                  <div className="flex items-start gap-3">
                    {showResponseLogo && (
                      <div className="flex-shrink-0 w-4 h-4 rounded-full bg-black/30 backdrop-blur flex items-center justify-center">
                    ...
                    <div className="flex-1">
                      {selectedLoadingAnimation && selectedLoadingAnimation.component ? (
                        selectedLoadingAnimation.component({ color: customLoadingColor })
                      ) : (
                        <div className="flex space-x-2">
                          <div className="w-2 h-2 rounded-full animate-bounce" style={{ backgroundColor: customLoadingColor }}></div>
                          <div className="w-2 h-2 rounded-full animate-bounce delay-100" style={{ backgroundColor: customLoadingColor }}></div>
                          <div className="w-2 h-2 rounded-full animate-bounce delay-200" style={{ backgroundColor: customLoadingColor }}></div>
                        </div>
                      )}
                    </div>
                  </div>
...
const LOADING_ANIMATIONS = [
  {
    name: "Spinner",
    component: ({ color }: LoadingAnimationProps) => (
      <div className="w-7 h-7 border-4 border-t-transparent rounded-full animate-spin" style={{ borderColor: `${color} transparent ${color} ${color}` }}></div>
    ),
  },
  {
    name: "Three Dots",
    component: ({ color }: LoadingAnimationProps) => (
      <div className="flex space-x-1">
        <span className="w-2 h-2 rounded-full animate-bounce [animation-delay:-0.3s]" style={{ backgroundColor: color }}></span>
        <span className="w-2 h-2 rounded-full animate-bounce [animation-delay:-0.15s]" style={{ backgroundColor: color }}></span>
  ...
  },
  {
    name: "Pulse",
    component: ({ color }: LoadingAnimationProps) => (
      <div className="w-5 h-5 rounded-full animate-pulse" style={{ backgroundColor: color }}></div>
    ),
  },
  {
    name: "Bar",
    component: ({ color }: LoadingAnimationProps) => (
      <div className="flex space-x-1 h-4 items-end">
        <div className="w-1 h-2 animate-[bar1_1s_infinite]" style={{animationDelay:'0s', backgroundColor: color }}></div>
        <div className="w-1 h-3 animate-[bar2_1s_infinite]" style={{animationDelay:'0.2s', backgroundColor: color }}></div>
        <div className="w-1 h-4 animate-[bar3_1s_infinite]" style={{animationDelay:'0.4s', backgroundColor: color }}></div>
  ...
  },
  {
    ...
    component: ({ color }: LoadingAnimationProps) => (
      <div className="flex space-x-1">
        <span className="w-2 h-2 rounded-full animate-bounce" style={{animationDelay:'0s', backgroundColor: color}}></span>
        <span className="w-2 h-2 rounded-full animate-bounce" style={{animationDelay:'0.1s', backgroundColor: color}}></span>
        <span className="w-2 h-2 rounded-full animate-bounce" style={{animationDelay:'0.2s', backgroundColor: color}}></span>
        <span className="w-2 h-2 rounded-full animate-bounce" style={{animationDelay:'0.3s', backgroundColor: color}}></span>
      </div>
    ),
  },
  {
  ...
  },
  {
    ...
    component: ({ color }: LoadingAnimationProps) => (
      <div className="flex space-x-1">
        <span className="w-1.5 h-4 rounded animate-bounce" style={{animationDelay:'0s', backgroundColor: color}}></span>
        <span className="w-1.5 h-4 rounded animate-bounce" style={{animationDelay:'0.1s', backgroundColor: color}}></span>
        <span className="w-1.5 h-4 rounded animate-bounce" style={{animationDelay:'0.2s', backgroundColor: color}}></span>
      </div>
    ),
  },
  {
    ...
    component: ({ color }: LoadingAnimationProps) => (
      <div className="flex space-x-2">
        <span className="w-3 h-3 rounded-full animate-pulse" style={{ animationDelay: "0s", backgroundColor: color }}></span>
        <span className="w-3 h-3 rounded-full animate-pulse" style={{ animationDelay: "0.2s", backgroundColor: color }}></span>
        <span className="w-3 h-3 rounded-full animate-pulse" style={{ animationDelay: "0.4s", backgroundColor: color }}></span>
      </div>
    ),
  },
  {
    name: "Growing Bar",
    component: ({ color }: LoadingAnimationProps) => (
      <div className="w-16 h-2 rounded animate-pulse" style={{ backgroundColor: color }}></div>
    ),
  },
  {
    name: "Rotating Square",
    component: ({ color }: LoadingAnimationProps) => (
      <div className="w-6 h-6 border-4 animate-spin rounded" style={{ borderColor: `${color} transparent ${color} transparent` }}></div>
    ),
  },
  {
    name: "Zigzag",
