...
const MODEL_OPTIONS = [
  { label: "Gemini 2.0 Flash", value: "gemini-1.5-flash-latest" },
  { label: "Gemma 3 12B", value: "gemma-1.5-12b-it" }
];

const TestGoogleChat: React.FC = () => {
  ...
  const handleSendMessage = async (text: string) => {
    ...
    setIsLoading(true);
    try {
      const res = await fetch("/api/text-google", {
        method: "POST",
      ...
      <div className="p-4 bg-black/20 border-b border-white/10">
        <div className="flex items-center space-x-3">
          ...
          <div className="flex-1">
            <h3 className="text-white text-xl font-semibold">Google Gemini Chatbot</h3>
            <p className="text-white/60 text-sm">Test Google Gemini Pro API with chat UI</p>
          </div>
          <div>
            <select
