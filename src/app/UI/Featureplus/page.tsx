...
const GeneratedComponent: React.FC = () => {  const [smallBubbles, setSmallBubbles] = useState<Bubble[]>([]);
  const [imageError, setImageError] = useState(false);
  const bubbleContainerRef = useRef<HTMLDivElement>(null);
  const isHovering = useRef(false);
  const animationFrameRef = useRef<number>();
  const startTimeRef = useRef<number>(Date.now());  useEffect(() => {
    const animate = () => {
      const currentTime = Date.now();
      const elapsedTime = (currentTime - startTimeRef.current) / 1000; // Convert to seconds

      setSmallBubbles(prev => prev.map(bubble => {
        if (isHovering.current) return bubble;

        // Individual rotation speeds for each axis
        const speedX = 0.3;
        const speedY = 0.4;
        const speedZ = 0.2;
        
        // Calculate rotation angles for each axis
        const angleX = elapsedTime * speedX;
        const angleY = elapsedTime * speedY;
        const angleZ = elapsedTime * speedZ;

        // Apply 3D rotation transformation
        let x = bubble.originalPos.x;
        let y = bubble.originalPos.y;
        let z = bubble.originalPos.z;

        // Rotate around X axis
        const temp1Y = y * Math.cos(angleX) - z * Math.sin(angleX);
        const temp1Z = y * Math.sin(angleX) + z * Math.cos(angleX);
        y = temp1Y;
        z = temp1Z;

        // Rotate around Y axis
        const temp2X = x * Math.cos(angleY) + z * Math.sin(angleY);
        const temp2Z = -x * Math.sin(angleY) + z * Math.cos(angleY);
        x = temp2X;
        z = temp2Z;

        // Rotate around Z axis
        const temp3X = x * Math.cos(angleZ) - y * Math.sin(angleZ);
        const temp3Y = x * Math.sin(angleZ) + y * Math.cos(angleZ);
        x = temp3X;
        y = temp3Y;
    ...
    const generateBubbles = () => {
      ...
      ] as const;      const bubbles = Array.from({ length: 100 }, (_, i) => {
        const phi = Math.acos(1 - 2 * (i + 0.5) / 100);
        const theta = Math.PI * 2 * i * (1 + Math.sqrt(5)) / 2;
          // Initial tight radius to cover the main bubble
        const coverRadius = 110; // Slightly larger radius for better rotation space
        ...
        <div className="container mx-auto px-6 relative z-10">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-16">
            ...
            <div className="lg:w-1/2 relative h-[700px] hidden lg:block" ref={bubbleContainerRef}>
              <div className="absolute left-1/2 top-1/3 transform -translate-x-1/2 -translate-y-1/2 perspective-[2000px]">                {/* Main 3D Bubble */}
                <div 
                  className="relative w-[200px] h-[200px] animate-float-slow transform-style-preserve-3d"
